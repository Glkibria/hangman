...
while(level != 1 && level!= 2 && level !=3)
 ...
 printf("\n\t\t\t\t*\t    5.HARD Level 1(Fruits and Sports)       *\n\t\t\t\t*\t    6.HARD level 2(Fruit and Sport and Tech)*\n\t\t\t\t*\t    7.HArd level 3(All)\t\t\t    *");
 printf("\n\t\t\t\t*****************************************************");
 printf("\n\t\t\t\t\t    Enter category :");
		scanf("%d", &category);
		...
		while(letterused== 1)
		{
			for(int i=  0; i< words; i++)
			{
				...
				else
				{
					...
					PlaySound(TEXT("F:\\mixkit-retro-game-notification-212.wav"),NULL,SND_ASYNC);
					scanf(" %c", &enterchar);
